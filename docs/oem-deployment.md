...
Впишите секцию `branding` в ваш конфиг-файл (какой именно файл — см. следующий
подраздел «Какой именно файл править»):
...
### Какой именно файл править

Приложение всегда читает конфиг из `config/app.json`. Но какой файл править вам —
зависит от способа развёртывания:

| Развёртывание | Файл, который правите вы | Почему |
|---------------|--------------------------|--------|
| Docker Compose (основной способ, `docker-compose.yml`) | `config/app.redis.json` | Compose монтирует этот файл внутрь контейнера **под именем** `config/app.json`. То есть `app.redis.json` и есть рабочий конфиг при compose-запуске. |
| Запуск без compose (образ вручную, свои скрипты) | `config/app.json` | Приложение читает его напрямую. |

Секция `branding` пишется одинаково в обоих случаях — разница между файлами
только в настройках хранилища сессий (`sessionStore`, Redis). Найдите в файле
секцию `branding` и впишите свои значения, либо добавьте её, если нет.
...
### Обязательные поля: правило «всё или ничего»

Кастомный бренд применяется **целиком или не применяется вообще**. Пять полей
обязательны — если хоть одно из них не задано (или осталось со значением
по умолчанию из поставки), продукт **игнорирует весь ваш бренд** и показывает
стандартный, а причину пишет в лог:

- `name` — название продукта (`BRAND_NAME`);
- `shortName` — короткое название (`BRAND_SHORT_NAME`);
- `applicationDomain` — домен приложения (`APPLICATION_DOMAIN`);
- `promotionWebsiteDomain` — домен сайта (`PROMOTION_WEBSITE_DOMAIN`);
- `senderEmail` — адрес отправителя писем (`BRAND_SENDER_EMAIL` или
  `APPLICATION_SENDER_EMAIL`).

Остальные поля (логотип, иконки, manifest, имя отправителя) можно не задавать —
у них нейтральные значения по умолчанию. Но проще всего задать сразу все: см.
пример выше.

Это защита от полуконфигурации: без правила продукт молча показывал бы смесь
«ваше название + чужие домены и письма», а с ним — либо полностью ваш бренд,
либо полностью стандартный, и в логе видно, чего не хватило.
...
повреждённая или отсутствующая OEM-лицензия возвращает стандартный бренд — не
через 14 дней, как у premium. Для конечных пользователей откат происходит тихо
(они видят стандартный бренд), а администратору он виден. Учтите: право
проверяется один раз при запуске процесса, поэтому смена состояния лицензии
(например, истечение) подхватится после перезапуска контейнера.
...
— или сработало правило «всё или ничего» (см. «Обязательные поля» выше).
...
5. заданы ВСЕ пять обязательных полей бренда (`name`, `shortName`,
   `applicationDomain`, `promotionWebsiteDomain`, `senderEmail`) и ни одно не
   осталось со значением по умолчанию. Если дело в этом, в логе при старте
   будет строка `[branding] OEM entitlement is valid but the brand config is
   INCOMPLETE` с перечнем незаданных полей.
