...
const isCommercialEdition = require('./edition/commercial_mode').isCommercialEdition;
const envResolver = require('./env_resolver');
const nodeEnv = process.env.NODE_ENV || 'development';
const isProductionLike = ['production', 'staging'].indexOf(nodeEnv) !== -1
  || isCommercialEdition();
...
// Settings that are each valid on their own but wrong in combination. Reported
// once at boot rather than thrown, because the shipped compose defaults would
// otherwise stop every existing deployment on upgrade. See lib/security_posture.js.
require('./security_posture').reportSecurityPosture({
  isProductionLike: isProductionLike,
  trustProxy: nconf.get('trust_proxy'),
  sessionCookieSecure: nconf.get('session_cookie_secure'),
});

// Single boot-time emit of the TIMEOFF_* → LEAVEPILOT_* deprecation warning
// (D-06). Idempotent: reportDeprecations guards on a module-level warned flag so
// re-requiring config never re-warns in the same process.
envResolver.reportDeprecations(process.env);
