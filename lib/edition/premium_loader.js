...
const resolveModuleName = () =>
  envResolver.resolve('PREMIUM_MODULE')
...
const isRequired = () => {
  if (typeof envResolver.resolve('PREMIUM_MODULE_REQUIRED') !== 'undefined') {
    return parseBoolean(envResolver.resolve('PREMIUM_MODULE_REQUIRED'));
  }
...
const load = ({registry, context, logger}) => {
  ...
  if (!moduleName) {
    if (required) {
      throw new Error('Premium module required but LEAVEPILOT_PREMIUM_MODULE is not configured.');
    }
  ...
  // Integrity check: verify premium module hash against license payload
  const licensePayload = features.readLicensePayload();
  const expectedHash = licensePayload.moduleHash;

  if (expectedHash) {
    const resolvedPath = (moduleName.indexOf('.') === 0 || moduleName.indexOf('/') === 0)
      ? path.resolve(process.cwd(), moduleName)
      : moduleName;
    const integrityResult = verifyModuleHash(resolvedPath, expectedHash);
