...
function _promise_to_unfold_context(context) {
  if (Object.prototype.hasOwnProperty.call(context, 'user')){
...
// Return Nodemailer's promise-based sendMail with its transport context intact.
...
Email.prototype.get_send_email = function(){
  ...
  return _transporter.sendMail.bind(_transporter);
};
...
Email.prototype.promise_leave_request_emails = function(args){
  ...
  return Promise.all([
    ...
    .then(([comments, requesterAllowance]) => {
      return leave.get("user").promise_supervisors()
        .then(supervisors => Promise.all(
          supervisors.map(supervisor => promise_email_to_supervisor({comments, requesterAllowance})(supervisor))
        ))
        .then(() => promise_email_to_requestor({comments, requesterAllowance}));
    });
};
...
Email.prototype.promise_upcoming_leave_start_reminder_email = function(args){
  ...
  const isEmployeeReminder = notification_type === 'leave_start_reminder_employee';
