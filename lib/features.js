...
const {generateFingerprint} = require('./machine_fingerprint');
const {readLicenseFile, getLicenseFilePath} = require('./license_storage');

// Production signing public key. This is the trust root — env overrides are
// only for key rotation. Embedded to prevent key-substitution attacks where
// an attacker with env access replaces the public key and signs their own
// license.
const HARDCODED_LICENSE_PUBLIC_KEY = '-----BEGIN PUBLIC KEY-----\n'
  + 'MIIBIjANBgkqhkiG9w0BAQEFAAOCAQ8AMIIBCgKCAQEAh5QLlBIGv4rA7H2DV+27\n'
  + 'OtIoUA/4QFmFckA+G2zK6hzvWNCl3UhVyGxYrSlNQkWJy+2EKNYOwgg6+ccx8Y3t\n'
  + 'Apb5DEErKWQ1zsHDF0Pd2GespzFDVbaz8dV3FL4fTWyAslEbXPYjzsq8WkAEuzzi\n'
  + 'uD/kZGBtfD9gOzCrQgKSjiRUDtSqN6RCLcoO+bTXPkK6ecgc4RMHOZ1p3P7JEEx2\n'
  + 'TiLa+uXOP7bhvQbCFjZOZGMGqjIOwx1jjPSmI72xJpOZUPjtMKrGgZhtKJencKg7\n'
  + '0Mio2iXnDF1SVZmwwSeJ8OOYPJP+URhyaotTMPYEnGB82a7SfD08wSDW3gB03daJ\n'
  + 'IQIDAQAB\n'
  + '-----END PUBLIC KEY-----';
...
const FEATURE_CATALOG = {
  ...
  // OEM-01: custom_branding is the white-label entitlement. It is a separate
  // tariff (D-01) carried in the signed payload.features, NOT a plan value in
  // plan_presets.json (D-02). The license-aware gate in lib/branding.js reads
  // it directly from getLicenseStatus(); it is never granted by grace (D-03).
...
const getLicenseSecret = () =>
  envResolver.resolve('LICENSE_SECRET') || config.get('license_secret') || '';

const getLicensePublicKey = () =>
  envResolver.resolve('LICENSE_PUBLIC_KEY') || config.get('license_public_key') || HARDCODED_LICENSE_PUBLIC_KEY;
...
const getLicensePublicKeyRing = () => {
  const raw = envResolver.resolve('LICENSE_PUBLIC_KEYS')
...
const getRevocationPublicKey = () =>
  envResolver.resolve('LICENSE_REVOCATION_PUBLIC_KEY')
  || config.get('license_revocation_public_key')
  || getLicensePublicKey();

const revocationListRawValue = () =>
  envResolver.resolve('LICENSE_REVOCATION_LIST')
...
const allowUnsignedLicenses = () => {
  ...
  // This default is the OEM gate's trust root (WR-01 fold, D-20): an
  // unsigned {"features":["custom_branding"]} payload activates the
  // white-label entitlement wherever this returns true. Only the test
  // environment grants by default; a development or unset NODE_ENV needs
  // the explicit ALLOW_UNSIGNED_LICENSES=true opt-in above.
...
const licenseRawValue = () => {
  if (envResolver.resolve('LICENSE')) {
    return {
      raw: envResolver.resolve('LICENSE'),
      source: 'env',
    };
  }
...
const getLicenseGraceDays = () => {
  const configured = Number(
    envResolver.resolve('LICENSE_GRACE_DAYS')
...
// Verify machine fingerprint in license payload. Returns null on success,
// error reason string on failure. Backward compatible: absent fingerprint
// means no hardware binding (returns null).
const verifyMachineFingerprint = payload => {
  if (!payload || !payload.machineFingerprint) {
    return null;
  }

  if (!/^[0-9a-f]{64}$/.test(payload.machineFingerprint)) {
    return 'invalid_machine_fingerprint';
  }
  ...
  if (!currentFingerprint) {
    ...
    console.warn('License: machine fingerprint collection failed; skipping hardware binding check');
    return null;
  }

  if (currentFingerprint !== payload.machineFingerprint) {
...
const verifyLicenseEnvelope = (license, source) => {
  ...
  if (algorithm === 'RSA-SHA256') {
    ...
    const fingerprintReason = verifyMachineFingerprint(payload);
...
const assertCommercialLicense = () => {
  ...
  if (!envResolver.resolve('LICENSE_PUBLIC_KEY') && !envResolver.resolve('LICENSE_PUBLIC_KEYS')) {
    throw new Error('Commercial mode requires TIMEOFF_LICENSE_PUBLIC_KEY or TIMEOFF_LICENSE_PUBLIC_KEYS.');
  }

  const parsedLicense = parseLicense(envResolver.resolve('LICENSE'));
...
const getRequestedFeatureSet = () => {
  const values = parseList(envResolver.resolve('FEATURES'));
