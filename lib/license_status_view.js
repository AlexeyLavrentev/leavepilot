...
//
// IMPORTANT (CONTEXT D-09): there is no separate short-term-license branch and
// no such UI bucket. Portal-issued short-term licenses carry plan:'enterprise'
// with no marker in the signed payload, so they are indistinguishable from a
// paid enterprise license at this layer. Such a license therefore flows through
// the normal valid-license path: it lands in the 'active' bucket when it has
// runway beyond the expiring threshold, or in the 'expiring' bucket when it is
// within the threshold (the countdown is covered by the existing expiring path).
...
function mapLicenseStatusToUiBucket(status, branding) {
  ...
  // 7. Otherwise -> active/valid (a portal short-term enterprise license, which
  //    carries plan:'enterprise' and no marker, also lands here; see D-09 above).
  return {
    tone: 'positive',
