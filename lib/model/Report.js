...
const getUsersWithLeaves = ({
  ...
  result = result.then(users => pMap(users, user => user.reload_with_leave_details({}), { concurrency : 10}));
...
const filterLeaves = ({startDate, endDate}) => (leave) => {
  const sd = dayjs.utc(leave.get_start_leave_day().date);
  const ed = dayjs.utc(leave.get_end_leave_day().date);
...
const leaveIntoObject = (leave) => {
  ...
  return {
    startDate:    dayjs.utc(leave.get_start_leave_day().date).format(dateFormat),
    endDate:      dayjs.utc(leave.get_end_leave_day().date).format(dateFormat),
    ...
    createdAt: dayjs.utc(leave.createdAt).format(dateFormat),
  };
};
