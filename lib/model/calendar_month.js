...
function CalendarMonth(day, args){
    const self = this;
    this.date           = dayjs.utc(day).startOf('month');
    this._leaves        = {};
    this._bank_holidays = {};
    ...
    if (args && args.today) {
      ...
      throw new Error('CalendarMonth requires today - dayjs object that represents today');
    }
    ...
    if (args && args.bank_holidays){
        ...
        args.bank_holidays.forEach(function(day){
            day = {
              date : dayjs.utc(day.date),
              name : day.name,
            };
            bankHolidayMap[day.date.clone().format(self.default_date_format())] = day;
        });
        self._bank_holidays = bankHolidayMap;
    }

    if (args && args.working_day_overrides){
        const workingDayOverrides = {};
        args.working_day_overrides.forEach(function(day){
            workingDayOverrides[dayjs.utc(day.date).format(self.default_date_format())] = true;
        });
    ...
    if (args && args.leave_days){
        ...
        args.leave_days.forEach(function(day){
            const attribute = dayjs.utc(day.date).format(self.default_date_format());
            if ( ! leaveDaysMap[attribute] ) {
                leaveDaysMap[attribute] = day;
            } else if ( leaveDaysMap[attribute] ) {

                if (leaveDaysMap[attribute].is_all_day_leave()) {
                    return;
                }

                if (day.is_all_day_leave()){
                    leaveDaysMap[attribute] = day;
                } else if ( leaveDaysMap[attribute].day_part !== day.day_part ) {

                  // Merge leave types from both days into one in "map"
                  if (day.is_morning_leave()) {
                    leaveDaysMap[attribute].morning_leave_type_id = day.morning_leave_type_id;
                  }

                  if ( day.is_afternoon_leave() ) {
                    leaveDaysMap[attribute].afternoon_leave_type_id = day.afternoon_leave_type_id;
                  }

                  leaveDaysMap[attribute].pretend_to_be_full_day();
                }
            }
        });
        self._leaves = leaveDaysMap;
    }
...
CalendarMonth.prototype.is_bank_holiday = function(day) {
  return this._bank_holidays && Object.prototype.hasOwnProperty.call(this._bank_holidays,
...
CalendarMonth.prototype.is_calendar_days_leave = function(day){
  const self = this;
  const ids = [ this.get_morning_leave_type_id(day), this.get_afternoon_leave_type_id(day) ];
  return ids.some(function(id){
    const lt = id && self._leave_types_map[ id ];
    if ( ! lt ) { return false; }
...
CalendarMonth.prototype.as_for_template = function(){
    ...
    for( let i=1; i<=self.how_many_days(); i++){
        const day = {
          val       : i,
          dayjs    : self.get_base_date().date(i),
...
CalendarMonth.prototype.as_for_team_view = function(){
  ...
  _.each(days, function(day){
    day.dayjs = me.get_base_date().date(day.val);
  });
