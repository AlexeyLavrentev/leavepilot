"use strict";

const dayjs = require('../../util/date'),
_          = require('underscore'),
config     = require('../../config');

module.exports = function(sequelize, DataTypes) {
    const BankHoliday = sequelize.define("BankHoliday", {
        name : {
            type      : DataTypes.STRING,
            allowNull : false,
        },
        date : {
          type         : DataTypes.DATE,
          allowNull    : false,
        },
        day_type : {
          type         : DataTypes.STRING,
          allowNull    : false,
          defaultValue : 'non_working',
        },
        import_source : {
          type      : DataTypes.STRING,
          allowNull : true,
        },
    }, {
        indexes : [
          {
            fields : ['companyId'],
          }
        ],
    });

    BankHoliday.associate = function( models ) {
        BankHoliday.belongsTo(models.Company, {as : 'company'});
        BankHoliday.belongsTo(models.WorkCalendar, {
          as         : 'work_calendar',
          foreignKey : 'workCalendarId',
        });
    };

    BankHoliday.generate_bank_holidays = function(args){
        const company = args.company,
            country_code = args.country_code;

        let bank_holidays = [
            {
                name : 'Early May bank holiday',
                date : '2015-05-04',
                companyId : company.id,
            },
        ];

        const config_countries = config.get('countries');

        if (
            Object.prototype.hasOwnProperty.call(config_countries, country_code)
            && Object.prototype.hasOwnProperty.call(config_countries[ country_code ], 'bank_holidays')
            && config_countries[ country_code ].bank_holidays.length > 0
        ) {
          bank_holidays = _.map(
            config_countries[ country_code ].bank_holidays,
            function(bh){
              return {
                name      : bh.name,
                date      : bh.date,
                companyId : company.id,
              };
            }
          );
        }

        return BankHoliday.bulkCreate(bank_holidays);
    };

    BankHoliday.prototype.get_pretty_date = function(){
      return dayjs.utc(this.date).format('YYYY-MM-DD');
    };

    return BankHoliday;
};
