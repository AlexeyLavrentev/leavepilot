...
const buildTeamViewDataCacheKey = ({companyId, departmentId, startDate, endDate, version, today}) =>
  teamViewCache.buildKey({
    ...
    today: dayjs.utc(today).format('YYYY-MM-DD'),
    version,
  });
...
const serializeTeamViewNode = node => ({
  ...
  days: node.days.map(day => ({
    ...day,
    dayjs: day.dayjs && dayjs.utc(day.dayjs).toISOString(),
...
const restoreTeamViewNode = node => ({
  ...
  days: node.days.map(day => ({
    ...day,
    dayjs: day.dayjs && dayjs.utc(day.dayjs),
...
module.exports = function(sequelize, DataTypes) {
  const Department = sequelize.define("Department", {
    ...
    // The boundary comes from the caller, which knows the company. Left to
    // itself the filter answers in UTC, and the team view then disagrees with
    // the employee's own page about whether they have left.
    Department.prototype.promise_active_users = function(today){
      return this.getUsers({
        scope: ["withDepartments", "withGroups"],
        where: sequelize.models.User.get_active_user_filter(today)
      });
    };
    ...
    Department.prototype._promise_team_view = function(args){
          ...
          .then(() => {
            ...
            if (dayjs.utc(end_date).format('YYYY') !== dayjs.utc(start_date).format('YYYY')) {
            ...
            if (dayjs.utc(start_date).dayOfYear() > dayjs.utc(end_date).dayOfYear()) {
              Exception.throw_user_error({
          ...
          .then(async function(company){
            ...
            const cacheKey = buildTeamViewDataCacheKey({
              ...
              startDate: dayjs.utc(start_date).startOf('month'),
              endDate: dayjs.utc(end_date).endOf('month'),
            ...
            const range_start = dayjs.utc(start_date).startOf('month');
            const range_end = dayjs.utc(end_date).endOf('month');

            // The company is loaded here already - the cache key below is keyed
            // on its today - so the filter is asked in the same clock.
            return self.promise_active_users(company.get_today())
            .then(function(users){
              return Promise.all(
              _.map(
                users,
            ...
            .then(function(users_and_leaves){
              ...
              const number_of_months = dayjs.utc(end_date).month() - dayjs.utc(start_date).month();
              ...
              users_and_leaves.forEach( user_data => {
                ...
                for ( let i=0; i<=number_of_months; i++ ) {
                  ...
                  const calendar_month = new CalendarMonth(
                    dayjs.utc(start_date).clone().add(i, 'months'),
                    {
                      bank_holidays :
