...
function LeaveRequest(args) {
    ...
    if (dayjs.utc(args.from_date).toDate() > dayjs.utc(args.to_date).toDate()){
...
LeaveRequest.prototype.is_within_one_day = function(){
    return dayjs.utc(this.from_date).format('YYYY-MM-DD')
          ===
        dayjs.utc(this.to_date).format('YYYY-MM-DD');
};
...
LeaveRequest.prototype._does_fit_with_point = function(leave_day, point_name){
    ...
    if (
        (
          dayjs.utc(leave_day.date).format('YYYY-MM-DD')
            ===
          dayjs.utc(this[point_name]).format('YYYY-MM-DD')
        )
          &&
