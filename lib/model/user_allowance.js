...
class UserAllowance {
  ...
  get number_of_days_available_in_allowance() {
    ...
    if (self.user.start_date &&
      dayjs.utc(self.user.start_date).year() > self._now.year()
    ) {
      return 0;
    }
  ...
  get employement_range_adjustment() {
    ...
    if (
      now.year() !== dayjs.utc(self.user.start_date).year()
      && ( ! self.user.end_date || dayjs.utc(self.user.end_date).year() > now.year() )
    ){
      return 0;
    }

    const start_date = dayjs.utc(self.user.start_date).year() === now.year()
      ? dayjs.utc(self.user.start_date)
      : now.clone().startOf('year'),
    end_date = self.user.end_date && dayjs.utc(self.user.end_date).year() <= now.year()
      ? dayjs.utc(self.user.end_date)
  ...
  get accrued_adjustment() {
    ...
    const period_starts_at = dayjs.utc(self.user.start_date).year() === now.year()
      ? dayjs.utc(self.user.start_date)
      : now.clone().startOf('year');

    const period_ends_at = self.user.end_date && dayjs.utc(self.user.end_date).year() <= now.year()
      ? dayjs.utc(self.user.end_date)
      : now.clone().endOf('year');

    const days_in_period = period_ends_at.diff( period_starts_at, 'days' );

    const delta = allowance * period_ends_at.diff( now, 'days' ) / days_in_period;
  ...
  static promise_allowance(args) {
    ...
    flow = flow.then(() => {
      ...
      if (forceNow && now) {
        args.now = now;
      } else if ( year && year.year() !== dayjs.utc().year()) {
