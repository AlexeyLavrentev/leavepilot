...
router.get('/:token/ical.ics', function(req, res){
  ...
  // readable in a subscriber's client.
  var brand = branding.get();
  cal.prodId({ company: brand.name, product: brand.shortName, language: 'EN' });
  ...
  Promise
    ...
    .then(async function(feed){
      ...
      */
      ...
      if (feed.is_calendar()){
        cal.name(brand.name + ': ' + user.full_name() + ' calendar');
        ...
        cal.name(`${ brand.name }: ${ user.full_name() }'s team whereabout`);
