...
router.get('/:token/ical.ics', function(req, res){
  ...
  const singleLine = function(v) { return String(v === null || v === undefined ? '' : v).replace(/[\r\n]+/g, ' ').trim(); };
  ...
  Promise
      ...
      if (feed.is_calendar()){
        ...
        return user
          ...
          .then(function(calendar){
            const days = _.flatten( calendar.map( cal => cal.as_for_team_view() ));
        ...
        return pMap(months, month => {
          ...
          .then(arrayOfDays => _.flatten(arrayOfDays));
    ...
    .then(async (days) => {
      for (const day of days) {
        ...
        const start = dayjs.utc(day.dayjs),
            end = dayjs.utc(day.dayjs);
        let allDay = false;
