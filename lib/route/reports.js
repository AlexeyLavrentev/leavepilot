...
router.get('/allowancebytime/', (req, res) => {
  ...
  ]).then(([team_view_details, company]) => {
      return team_view
        .inject_statistics({
...
const sortersForLeavesReport = {
  ...
  startDate: (a,b) => dayjs.utc(a.startDate).toDate().valueOf() - dayjs.utc(b.startDate).toDate().valueOf(),
  endDate: (a,b) => dayjs.utc(a.endDate).toDate().valueOf() - dayjs.utc(b.endDate).toDate().valueOf(),
  status: (a,b) => sorter(a.status, b.status),
  createdAt: (a,b) => dayjs.utc(a.createdAt).toDate().valueOf() - dayjs.utc(b.createdAt).toDate().valueOf(),
...
router.get('/leaves/', async (req, res) => {
  ...
  try {
    ...
    leaves.sort(getSorterForLeaves(sortBy));
  } catch (error) {
    log.error(
  ...
  if (renderAsCsv) {
    return renderLeavesReportAsCsv({res, company, startDate, endDate, leaves});
  }
    return res.render('report/leaves', {
