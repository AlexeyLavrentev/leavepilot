...
router.post('/add/', function(req, res){
  ...
  .catch(function(error){
    ...
    log.error('add user error', { err: error.message });
...
router.post('/import/', function(req, res){
  ...
  Promise.resolve(form.parse(req))
    .then(([fields, files]) => {
      ...
      return fs.readFile(uploaded.filepath || uploaded.path, "utf8");
    })
...
router.get('/edit/:user_id/', function(req, res){
  ...
  Promise.resolve()
      .then(() => ensure_user_id_is_integer({req : req, user_id : user_id}))
...
router.get('/edit/:user_id/absences/', function(req, res){
  ...
  .then(employee => Promise.all([

    employee
      .promise_allowance()
      .then( allowance_obj => { user_allowance = allowance_obj; return Promise.resolve([user_allowance, employee]); } ),

    employee
      .promise_adjustmet_for_year(dayjs.utc().format('YYYY')),

    employee
      .promise_carried_over_allowance_for_year(dayjs.utc().format('YYYY')),

  ]).then(([args, employee_adjustment, carried_over_allowance]) => {
      args.push(null);
...
router.get('/edit/:user_id/schedule/', function(req, res){
  ...
  .catch(function(error){
    log.error(
      'An error occurred when trying to open employee schedule by user '+req.user.id
      + ' : ' + error
    );
...
router.get('/edit/:user_id/calendar/', async (req, res) => {
  ...
  return res.render('user_details', {
    ...
    previous_year  : dayjs.utc(year).add(-1,'year').format('YYYY'),
    current_year   : year.format('YYYY'),
    next_year      : dayjs.utc(year).add(1,'year').format('YYYY'),
    show_full_year : true,
...
//
const ensure_user_was_not_useed_elsewhere_while_being_inactive = function(args){
  const
    ...
    (
     ...
     ||
      (
        ...
        new_user_attributes.end_date &&
        dayjs.utc( new_user_attributes.end_date ).startOf('day').toDate() >= req.user.company.get_today().startOf('day').toDate()
      )
    )
  ) {
    return model.User.find_by_email(new_user_attributes.email)
      .then(function(user){
        ...
        if (user && user.companyId !== employee.companyId) {
          const error_msg = req.t('users.messages.emailExistsOtherAccount');
...
//
const ensure_email_is_not_used_elsewhere = function(args){
  const
    employee            = args.employee,
    new_user_attributes = args.new_user_attributes,
    req                 = args.req,
...
const ensure_we_are_not_removing_last_admin = function(args){
  const
    employee            = args.employee,
    new_user_attributes = args.new_user_attributes,
    req                 = args.req,
...
router.post('/delete/:user_id/', function(req, res){
    ...
    Promise.resolve().then(() => ensure_user_id_is_integer({req, user_id}))
    .then(() => req.user.get_company_for_user_details({user_id}))
    .then(company => {
      const employee = company.users[0];
...
router.all('/search/', function(req, res){
  ...
  return promise_result.then(function(company){
    ...
    log.error('Failed to search company users: ' + error);
...
router.get('/', function(req, res) {
    ...
    .then(function(company){
      return Promise.all(company.users.map(function(user){
    ...
    .then(company => pMap(
      ...
      { concurrency : 10 }
    ).then(users_info => [company, users_info]))
...
function promise_user_list_data_for_rendering(args) {
  ...
  const usersInfoForRendering = users_info.map(ui => ({
    ...
    // The company is in scope here, so the boundary is passed rather than left
    // to the association, which this query does not load onto the user rows.
    is_active                             : ui.user_row.is_active(company.get_today()),
  }));
...
function users_list_as_csv(args) {
  ...
  const content = [['email', 'lastname', 'name', 'department', 'remaining allowance', 'days used']];
...
function get_and_validate_user_parameters(args) {
    ...
    let email    = validator.trim(req.body.email_address);
    const department_id     = validator.trim(req.body.department);
    let start_date        = validator.trim(req.body.start_date),
        end_date          = validator.trim(req.body.end_date);
    const adjustment        = validator.trim(req.body.adjustment),
    ...
    if (
        start_date &&
        end_date &&
        dayjs.utc(start_date).toDate() > dayjs.utc(end_date).toDate()
    ){
