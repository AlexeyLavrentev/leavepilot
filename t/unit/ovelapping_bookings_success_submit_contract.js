...
describe('overlapping booking successful submit contract', function(){
  ...
  it('uses normal same-URL redirect synchronization and waits for the leave modal to close', function(){
    ...
    expect(successfulRequest).to.match(/submit_form_func\(\{[\s\S]*?message\s*:\s*\/New leave request was added\/[\s\S]*?modal_selector\s*:\s*['"]#book_leave_modal['"][\s\S]*?\}\)/);
